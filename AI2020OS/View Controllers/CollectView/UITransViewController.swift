...
class UITransViewController: UIViewController {
    ...
    private var swipeView: AISwipeView!
 //   private var indicator: JTSScrollIndicator!
...
extension UITransViewController: UIScrollViewDelegate {
   ...
   //    indicator.scrollViewDidScroll(scrollView)
        ...
        if let bar = scrollView.subviews.last as UIImageView? {
            bar.image = nil
        }

        if swipeView.panel.frame.origin.x == 98.0 {
            //hidden
            swipeView.setWidth(80.0)
            swipeView.setLeft(self.tableView.width - 80.0)
...
extension UITransViewController: SwipeableCellDelegate{
    ...
    func cellDidAimationFrame(position: CGFloat, cell: UITableViewCell!) {
        ...
        if viewtag > 0 {
            ...
            if let veiwTagsMore = superCell.view_Content.viewWithTag(5) {
...
//        })
    ...
    func cellDidClose(cell: UITableViewCell!) {
        ...
        for index  in 1...6 {
            ...
            if let tagView = superCell.view_Content.viewWithTag(5) {
                if let button = tagView.viewWithTag(index) as? UIButton{
                    button.setImage(UIImage(named: "scrollball_\(index)"), forState: UIControlState.Normal)
                    if index == 6 {
                        button.setImage(UIImage(named: "scrollball_\(index - 1)"), forState: UIControlState.Normal)
                    }
        ...
        if let colorssArray  =  model.colors{
            for colorss in colorssArray {
                ...
                if let tagView = superCell.view_Content.viewWithTag(5) {
                    var indsx = colorss.intValue()
                    if indsx == 5 {
                        indsx = 6
                    }
                    if let button = tagView.viewWithTag(indsx) as? UIButton{
...
extension UITransViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell{
        ...
        for index  in 1...6 {
            ...
            if let tagView = cell.view_Content.viewWithTag(5) {
                if let button = tagView.viewWithTag(index) as? UIButton{
                    button.setImage(UIImage(named: "scrollball_\(index)"), forState: UIControlState.Normal)
                    if index == 6 {
                        button.setImage(UIImage(named: "scrollball_\(index - 1)"), forState: UIControlState.Normal)
        ...
        if model.favoriteType == FavoriteTypeEnum.music.value() {
            ...
            if model.ctExpand == .Collapsed {
                ...
                tView.tag = View_Tag.View_TOP.rawValue
                ...
                actionView.tag = View_Tag.View_ACTION.rawValue
        ...
        if model.favoriteType == FavoriteTypeEnum.image.value() || model.favoriteType == FavoriteTypeEnum.video.value() {
            ...
            if model.ctExpand == .Collapsed {
                ...
                mediaView.tag = View_Tag.View_MEDIA.rawValue
                ...
                contentView.tag = View_Tag.View_CONTENT.rawValue
    ...
    private func heightForContent(content:String?) -> CGFloat{
        let size = CGSizeMake(self.view.width - 60,1000)
        let s:NSString = "\(content)"
        let contentSize = s.boundingRectWithSize(size, options: NSStringDrawingOptions.UsesLineFragmentOrigin , attributes: [NSFontAttributeName:UIFont.systemFontOfSize(10)], context: nil)
    ...
    private func fillAITopInfoView(model: AITransformContentModel,topView:AITopInfoView,indexPath: NSIndexPath){
        ...
        */
        ...
        /**
        if model.ctExpand == .Collapsed {
        topView.Label_Title.textColor = UIColor.whiteColor()
        topView.Label_Type.textColor = UIColor.whiteColor()
        }else{
        topView.Label_Title.textColor = UIColor.blackColor()
        topView.Label_Type.textColor = UIColor.blackColor()
        }
        */
    }
    ...
    func onAction(section: Int, actionType: ActionType) {
        ...
        case ActionType.Browse:
            ...
            if let url = model.favoriteFromWhereURL {
                ...
                webView.currentUrl = NSURL(string: "")
            }
            ...
            */
            if let url = model.favoriteFromWhereURL {
                ...
                showViewController(webVC, sender: self)
            }
