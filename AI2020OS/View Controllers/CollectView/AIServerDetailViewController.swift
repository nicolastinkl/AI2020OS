...
class AIServerDetailViewController: UIViewController {
    ...
    var orderManager = AIOrderRequester()
    ...
    @IBAction func submitOrderAction(sender: UIButton) {
        //AIViews.showAnimationLoadingOnView(self.view)
//         orderManager.submitOrder(201505261427, completion: { (success) -> Void in
//            //let result = AIViews.stopAnimationLoadingOnView(self.view)
//            SCLAlertView().showInfo("提示", subTitle: "订单保存成功", closeButtonTitle: "关闭", duration: 3)
//         })
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        if dataSource.count > indexPath.row{
            ...
            if  model.type == cellType.cellTypeparames {
                ...
                if Int(dataSource.count - 1) == indexPath.row {
